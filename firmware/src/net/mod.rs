...
pub fn load_kernel() -> Vec<u8> {
    ...
    let device = unsafe {
        XilinxAxiEthernet::new(
            crate::address::ETH_MAC_BASE,
            crate::address::ETH_DMA_BASE,
            crate::address::MAC_ADDRESS,
        )
    };

    let ethernet_addr = EthernetAddress(crate::address::MAC_ADDRESS);
